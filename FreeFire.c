/*
  Nível Mestre - Sistema de Prioritização e Montagem de Componentes
  Implementa:
    - Struct Componente { nome[30], tipo[20], prioridade }
    - Cadastro de até 20 componentes
    - Bubble Sort por nome (string) - com contador de comparações
    - Insertion Sort por tipo (string) - com contador de comparações
    - Selection Sort por prioridade (int) - com contador de comparações
    - Busca Binária por nome (aplicável após ordenar por nome) - conta comparações
    - Medição de tempo de execução usando clock()
...
// Enum para rastrear última ordenação aplicada
...
// Lê uma string com fgets de forma segura e remove newline
...
/* -------------------- Algoritmos de ordenação (instrumentados) --------------------
   Cada algoritmo recebe:
     - arr[] e n
     - ponteiro para contador de comparações (long long *) que a função incrementa
   Retorna via parâmetros: arr ordenado, comparações preenchidas.
*/

/* Bubble sort por nome (ordenação alfabética ascendente).
   Conta comparações de string (cada strcmp entre elementos é considerada 1 comparação). */
...
/* Insertion sort por tipo (ordena por campo tipo, em ordem alfabética).
   Contagem: cada comparação de string (strcmp) incrementa o contador. */
...
/* Selection sort por prioridade (ordena por prioridade decrescente - maior prioridade primeiro).
   Contagem: cada comparação de prioridade (int) incrementa o contador. */
...
/* -------------------- Busca binária por nome (aplicável apenas em arr ordenado por nome) -------------
   Retorna índice do elemento encontrado ou -1 se não encontrado.
   Conta comparações: cada comparação de strcmp com o elemento do meio conta 1 comparação.
   Observação: strcmp retorna 0 quando iguais; se resultado <0 => chave < meio.
*/
...
/* -------------------- Função medirTempo --------------------
   Mede tempo de execução (em segundos) de um algoritmo que tenha assinatura:
     void algoritmo(Componente[], int, long long *)
   Parâmetros:
     - func: ponteiro para função do tipo acima
     - arr: vetor de componentes (será passado uma cópia externa se não quisermos alterar original)
     - n: tamanho
     - comparacoes: ponteiro para long long que receberá número de comparações
...
double medirTempo(AlgOrdenacao func, Componente arr[], int n, long long *comparacoes) {
    // Fazemos uma cópia local para que a função de medição não altere o vetor externo
    ...
    // Copiamos o resultado ordenado de volta para arr (comportamento: medirTempo também atualiza arr)
...
void cadastrarComponentes() {
    ...
    // pedimos número de componentes a cadastrar
    while (1) {
    ...
    ultimaOrdenacao = NENHUMA; // nova inserção quebra garantia de ordenação
    printf("\n Cadastro concluido. Total de componentes: %d\n", totalComponentes);
    mostrarComponentes(componentes, totalComponentes);
}

/* -------------------- Funções de interface para ordenar e buscar -------------------- */
...
int main() {
    ...
    printf("Bem-vindo ao modulo avançado de montagem da torre de fuga!\n");
